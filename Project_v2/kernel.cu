...
Matrix Affine1_ReLU(Matrix input, Matrix W)
{
    ...
    int block_x = TILE_W_AFFINE1; int block_y = TILE_H_AFFINE1;
    int grid_x = INPUT_NUM_PER_COMPUTE;
...
Matrix Affine2(Matrix input, Matrix W)
{
    ...
    // This matrix multiplication is a (1 x m) * (m x n) computation
    // The input is a 1 dimension matrix (with large number of elements)
    int block_x = TILE_W_AFFINE2; int block_y = TILE_H_AFFINE2;
    int grid_x = INPUT_NUM_PER_COMPUTE;
    dim3 blockDim(block_x, block_y, 1);
    dim3 gridDim(grid_x, 1, 1);
...
{
    // Shared memory
    __shared__ float X_p[TILE_H_AFFINE1];
    __shared__ float W_p[TILE_H_AFFINE1][TILE_W_AFFINE1];
    __shared__ float output_s[W2_WIDTH];
    
    // Indexing
    int tx = threadIdx.x; int ty = threadIdx.y;
    int bx = blockIdx.x;
    int phase_total = W.height / TILE_H_AFFINE1;
    if (W.height % TILE_H_AFFINE1) {phase_total++;}

    int num_matrix = input.num_matrix; // The number of image being processed
    int num_element_i = input.height * input.width; // A size of each input matrix
    int num_total_element_i = num_element_i * input.channel * input.num_matrix; // Total number of input elements 
    int num_element_o = output.height * output.width; // A size of each output matrix       

    // Initialize the output_s array to all zero
    if(tx < W2_WIDTH)
         output_s[tx] = 0.0f;
    ...
    for (int phase_n = 0; phase_n < phase_total; phase_n++){
        // Load data to Xp
        int idx_input = bx * num_element_i + TILE_H_AFFINE1 * phase_n + ty;
        if (idx_input < num_total_element_i)  
            X_p[ty] = input.elements[idx_input];
        else
            X_p[ty] = 0.0f;
        ...
        // Load data to Wp
        int idx_W_y = phase_n * TILE_H_AFFINE1 + ty;
        if(idx_W_y < W2_HEIGHT){
            W_p[ty][tx] = W.elements[phase_n * TILE_H_AFFINE1 * TILE_W_AFFINE1 + ty * TILE_W_AFFINE1 + tx];      
        }
        else
            W_p[ty][tx] = 0.0f;
        ...
        for (int row_w = 0; row_w < TILE_H_AFFINE1; row_w++){
            value = X_p[row_w] * W_p[row_w][tx];
            output_s[tx] += value; // Accumulate the result to the shared memory, output_s
        }
    ...
    // Add bias and ReLU
    int idx_output = num_element_o * bx + tx;
    //value = output[idx_output];
    value = output_s[tx]; // Get the value from the shared memory, output_s
    value += b2_d[tx]; // Add Bias2
    if(value < 0) // ReLU
        value = 0.0f;
    output.elements[idx_output] = value; // Assign the final result to the global memory, output
    //output.elements[idx_output] = 1.0;  
...
{
    // Shared memory
    __shared__ float X_p[TILE_H_AFFINE2];
    __shared__ float W_p[TILE_H_AFFINE2][TILE_W_AFFINE2];
    __shared__ float output_s[W3_WIDTH];

    // Indexing
    int tx = threadIdx.x; int ty = threadIdx.y;
    int bx = blockIdx.x;
    int phase_total = W.height / TILE_H_AFFINE2;
    if (W.height % TILE_H_AFFINE2) {phase_total++;}
    ...
    for (int phase_n = 0; phase_n < phase_total; phase_n++){
        // Load data to Xp
        int idx_input = bx * num_element_i + TILE_H_AFFINE2 * phase_n + ty;
        if (idx_input < num_total_element_i)
            X_p[ty] = input.elements[idx_input];
        else
            X_p[ty] = 0.0f;
        ...
        // Load data to Wp
        int idx_W_y = phase_n * TILE_H_AFFINE2 + ty;
        if(idx_W_y < W3_HEIGHT){
            W_p[ty][tx] = W.elements[phase_n * TILE_H_AFFINE2 * TILE_W_AFFINE2 + ty * TILE_W_AFFINE2 + tx];
        }
        else
            W_p[ty][tx] = 0.0f;
        ...
        for (int row_w = 0; row_w < TILE_H_AFFINE2; row_w++){
            value = X_p[row_w] * W_p[row_w][tx];
            output_s[tx] += value; // Accumulate the result to the shared memory, output_s
        }
